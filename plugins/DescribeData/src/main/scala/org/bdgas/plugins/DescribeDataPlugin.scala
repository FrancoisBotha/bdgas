...
object DescribeDataPlugin extends SparkSessionJob with NamedObjectSupport {
  ...
  def runJob(sparkSession: SparkSession, runtime: JobEnvironment, data: JobData): JobOutput = {
    ...
    val dummy2     = data(2) //Not used


    val a = sparkSession.sql(s"SELECT * FROM $fileAlias").describe().toJSON
